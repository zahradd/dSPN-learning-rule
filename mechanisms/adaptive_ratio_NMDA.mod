...
NEURON {
	POINT_PROCESS adaptive_ratio_NMDA
	RANGE g, Alpha, Beta, Erev, gmax, Cdur, iNMDA,mg, Cmax, eta, alpha,w0,  treshf,synon,flagx,conc0,mltypeMin,mltype, rate_ltd,rate_ltp,rate_ltd_thrsh,rate_ltp_tresh,tremin,ratio_0
...
STATE {Ron Roff weight tresh ratio}
...
BREAKPOINT {
    ...
    ratio=ratio+sratio(dopa,tresh,conc0,mltype,ratio)
...
:		 come again in Cdur with flag = current value of nspike
	...
	if (flag == nspike) { : if this associated with last spike then turn off
		r0 = (weight*(ratio))*Rinf + (r0 - (weight*(ratio))*Rinf)*exp(-(t - t0)/Rtau)
		t0 = t
		synon = synon - (weight*(ratio))
		state_discontinuity(Ron, Ron - r0)
		state_discontinuity(Roff, Roff + r0)
...
FUNCTION sTresh(dopam,tresh(mM),conc0,mltype)(uM/ms) {
    UNITSOFF
    sTresh =((-rate_ltd_thrsh*funcCalMin2(mltype)*funcCalt(conc0,tresh)*(dopam-1))-rate_ltp_tresh*(funcCalMin2(mltype)*funcCalt(conc0,tresh)*(dopam+1)))*trap(g)*dopam*dopam
	UNITSON    
}

FUNCTION sratio(dopam,tresh(mM),conc0,mltype,ratio)(uM/ms) {
    UNITSOFF
    sratio =(((-rate_ltd_thrsh*funcCalMin2(mltype)*funcCalt(conc0,tresh)*(dopam-1))-rate_ltp_tresh*(funcCalMin2(mltype)*funcCalt(conc0,tresh)*(dopam+1)))*trap(g)*dopam*dopam)*8*(3.2-ratio)*ratio
	UNITSON    
}
